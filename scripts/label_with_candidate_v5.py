#!/usr/bin/env python3
"""Offline PUSULA Semantic Candidate V5 batch labeler.

Candidate V5 uses a single frozen multilingual-e5-base embedding pass, then
keeps intent and clickbait as separate supervised/nonparametric targets:

- canonical 4D intent vector: cosine-distance k-NN over 352 V4 intent rows
  (256 clear + 64 neutral/mixed + 32 targeted hard negatives)
- auxiliary dominant intent: logistic regression over the 256 clear rows;
  diagnostics only, never used as the ranking target
- continuous clickbait severity: Ridge(alpha=0.05) over 192 presentation rows
  (64 manipulative + 128 clean, including 64 normal-feed hard negatives)

The clickbait head predicts the same continuous 0..1 quality signal consumed by
PUSULA's existing `quality = 1 - clickbait` formula; it is not a balanced-class
probability. The web runtime consumes cached outputs and does not run E5.

All architecture-selection metrics remain development evidence. Final
competition claims require a separately annotated untouched holdout.
"""
...
CLICKBAIT_FEED_HARDNEG = ROOT / "data/v4_calibrated/clickbait_feed_hardneg_v2.jsonl"
MODEL_NAME = "intfloat/multilingual-e5-base"
METHOD = "pusula-semantic-candidate-v5"
...
def load_clickbait_rows():
    rows = []
    for path in CLICKBAIT_BASE + [CLICKBAIT_FEED_HARDNEG]:
        for r in read_jsonl(path):
            rows.append(
                {
                    "id": str(r["id"]),
...
def screening_score(probs, nearest, ood_threshold):
    ...
    cls_margin = np.clip(ordered[:, -1] - ordered[:, -2], 0.0, 1.0)
...
def main():
    ...
    if any(r["auxiliary"] is not None for r in neutral + hardneg):
        raise SystemExit("neutral/hard-negative intent rows must not carry auxiliary hard labels")
    if len(click_train) != 192 or Counter(r["label"] for r in click_train) != Counter({0: 128, 1: 64}):
        raise SystemExit("clickbait training set must be 192 rows: 64 positive / 128 negative")

    # Shared encoder pass across both heads and the incoming batch.
    ...
    click_target = np.asarray([r["target"] for r in click_train], dtype=float)
    click_ridge = Ridge(alpha=0.05)
    click_ridge.fit(x_click, click_target)
    click_score = np.clip(np.asarray(click_ridge.predict(x_input), dtype=float), 0.0, 1.0)
    ...
    intent_margin = np.clip(ordered[:, -1] - ordered[:, -2], 0.0, 1.0)
    ...
    with out.open("w", encoding="utf-8") as f:
        for i, row in enumerate(input_rows):
            payload = {
                ...
                "clickbait": round(float(click_score[i]), 6),
                ...
                "diagnostics": {
                    ...
                    "clickbait_head": "e5_ridge_alpha_0.05_continuous_192",
                    "classifier_training_rows": 256,
                    "vector_training_rows": 352,
                    "clickbait_training_rows": 192,
                    "clickbait_positive_rows": 64,
                    "clickbait_negative_rows": 128,
    ...
    summary = {
        ...
        "mean_clickbait": float(np.mean(click_score)),
        "median_clickbait": float(np.median(click_score)),
        "p90_clickbait": float(np.quantile(click_score, 0.90)),
        "p95_clickbait": float(np.quantile(click_score, 0.95)),
        "max_clickbait": float(np.max(click_score)),
        "clickbait_ge_0_50": int(np.sum(click_score >= 0.50)),
        "clickbait_ge_0_65": int(np.sum(click_score >= 0.65)),
        "output": str(out),
    }
