...
const U=x=>{try{const u=new URL(String(x||''));return u.protocol==='https:'?E(u.href):'#'}catch(e){return '#'}};
const D=x=>{try{return new Intl.DateTimeFormat('tr-TR',{day:'numeric',month:'long',year:'numeric'}).format(new Date(String(x)+'T12:00:00Z'))}catch(e){return String(x||'')}};
...
function ff(){let a=G.f,q=G.q.trim().toLocaleLowerCase('tr-TR');if(G.tab==='media')a=a.filter(p=>MEDIA_CATS.has(p.kategori));if(q)a=a.filter(p=>((p.headline||'')+' '+p.metin+' '+p.yazar+' '+p.kategori_adi+' '+(p.source_name||'')).toLocaleLowerCase('tr-TR').includes(q.replace(/^#/,'')));return a}
function pc(p){
  let fit=S.intent&&S.mode==='pusula'?Math.round(p.fit*100)+'% niyet uyumu':'';let id=E(p.id),news=p.style==='verified_news';
  const newsHead=news?`<div class="newsKicker"><span>Haber</span><time datetime="${E(p.news_date)}">${E(D(p.news_date))}</time></div><h2 class="newsHeadline">${E(p.headline)}</h2>`:'';
  const newsSource=news?`<div class="newsSource"><span>✓ Doğrulanmış kaynak: <b>${E(p.source_name)}</b></span><a href="${U(p.source_url)}" target="_blank" rel="noopener noreferrer">Kaynağı aç ↗</a></div>`:'';
  return `<article class="post ${news?'newsPost':''}" data-post-id="${id}"><div class="postInner"><div class="avatar ${AC(p.id)}">${E(p.initials)}</div><div class="postMain"><div class="head"><span class="name">${E(p.yazar)}</span><span class="handle">· ${E(p.kategori_adi)}</span><button class="moreBtn" onclick="toast('Gönderi ${id}')">${svg('more')}</button></div>${newsHead}<div class="postText">${E(p.metin)}</div>${newsSource}<div class="postMeta">${fit?`<span class="fitTag">${fit}</span>`:''}<button class="why" onclick="reason('${id}')">Neden bunu görüyorum?</button></div><div class="reason" id="r${id}"><b>${E(why(p))}</b><div class="raw">sıra #${p.rank} · skor ${F(p.score)} · uyum ${F(p.fit)} · kalite ${F(p.quality)} · tazelik ${F(p.tazelik)} · etkileşim ${F(p.etkilesim_puani)} · clickbait ${F(p.clickbait)}</div></div><div class="postActions"><button class="act" aria-label="Yorum yap" onclick="toggleComment('${id}')">${svg('comment')}<span>${p.comment_count}</span></button><button class="act ${p.viewer_shared?'on':''}" aria-label="Yeniden paylaş" onclick="interact('${id}','share',this)">${svg('repeat')}<span>${p.share_count}</span></button><button class="act ${p.viewer_liked?'on liked':''}" aria-label="Beğen" onclick="interact('${id}','like',this)">${svg('heart')}<span>${p.like_count}</span></button><button class="act" aria-label="Bağlantıyı paylaş" onclick="copyPost('${id}')">${svg('share')}</button></div><div class="commentComposer" id="comment-${id}"><input maxlength="240" placeholder="Yorumunu yaz…" onkeydown="if(event.key==='Enter'){event.preventDefault();submitComment('${id}',this)}"><button onclick="submitComment('${id}',this.previousElementSibling)">Gönder</button></div></div></div></article>`
}
...
function why(p){const interaction=p.metadata_simulated?'Başlangıç etkileşimi simüle; kullanıcı hareketleri canlı güncellenir.':'Etkileşim canlı sayaçlardan hesaplanır.',source=p.style==='verified_news'?`${p.source_name} kaynaklı, tarihli haber özeti. `:'';if(!S.intent)return`${source}Klasik sıralamada #${p.rank}. ${interaction} Skor ${F(p.score)}.`;if(S.mode==='classic')return`${source}Klasik sıralama açık; ${I[S.intent].label} niyeti skora dahil değil. ${interaction} Skor ${F(p.score)}.`;return`${source}${I[S.intent].label} niyetinle %${Math.round(p.fit*100)} uyumlu. ${interaction} Kalite çarpanı ${F(p.quality)}.`}
