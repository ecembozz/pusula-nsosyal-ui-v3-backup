...
function isTimeoutError(err){
  ...
  return name==='aborterror' || err?.code==='ETIMEDOUT' || message.includes('aborted') || message.includes('zaman aşımı');
}
...
async function requestAnalysis({key,model,content,schema,timeoutMs=REQUEST_TIMEOUT_MS}){
  ...
  try{
    ...
    try{
      ...
      if(controller.signal.aborted){
        const timeoutError=new Error('Gemini zaman aşımı: '+model+' ('+timeoutMs+' ms)');
...
async function analyzePost(content){
  ...
  const models=[...new Set([configured,DEFAULT_MODEL,configuredFallback,FALLBACK_MODEL].filter(Boolean))];
  const timeoutMs=Math.max(3000,Math.min(20000,Number(process.env.GEMINI_TIMEOUT_MS)||REQUEST_TIMEOUT_MS));
  const schema={
    type:'object',
    properties:{
  ...
  for(let i=0;i<models.length;i++){
    ...
    try{
      return await requestAnalysis({key,model,content,schema,timeoutMs});
    }catch(err){
      lastError=err;
      if(err?.status===401 || err?.status===403) throw err;
      if(i===models.length-1 || !canFallback(err)) break;
      await sleep(200);
    }
  }
...
module.exports={
  CATEGORY_KEYS,DEFAULT_MODEL,FALLBACK_MODEL,REQUEST_TIMEOUT_MS,
  analyzePost,normalizeAnalysis,retryableStatus,isTimeoutError,canFallback
};
